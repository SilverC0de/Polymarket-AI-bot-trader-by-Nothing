...
type TraderConfig struct {
	...
	// ProxyWallet is the 0x-prefixed proxy wallet address (sig types 0, 1, 2 only).
	// This is where your pUSD sits for the old proxy/Safe flow.
	ProxyWallet string
	// DepositWallet is the 0x-prefixed deposit wallet address (sig type 3 / POLY_1271).
	// Required for new API users following the deposit wallet migration.
	// Both order maker and signer fields are set to this address.
...
func NewTrader(config TraderConfig) (*Trader, error) {
	...
	if config.SignatureType == SigTypePoly1271 {
		if !strings.HasPrefix(config.DepositWallet, "0x") {
			return nil, fmt.Errorf("POLYMARKET_DEPOSIT_WALLET must start with 0x for POLY_1271 (signature type 3)")
		}
	} else {
...
func (t *Trader) PlaceMarketOrder(ctx context.Context, tokenID string, amountUSD, entryPrice float64) (*OrderResult, error) {
	...
	// For all other sig types, maker = proxy wallet, signer = EOA.
	var makerAddr, signerAddr common.Address
	if t.config.SignatureType == SigTypePoly1271 {
		makerAddr = common.HexToAddress(t.config.DepositWallet)
...
func (t *Trader) signOrderPoly1271(o *v2Order) (string, error) {
	depositWallet := common.HexToAddress(t.config.DepositWallet)
